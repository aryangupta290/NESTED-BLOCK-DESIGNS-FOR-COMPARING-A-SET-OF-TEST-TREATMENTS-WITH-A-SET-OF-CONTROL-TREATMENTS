...
def method6(bib_design, v2):
    ...
    if (v_ > b_):
        ...
        return -1
    if (v2 % 2 == 1):
        print("Error: Number of Control treatments should be even")
        return -1

    # check if (v-1)*(lambda1 - q*lambda2) = (q-1)*r
    if (k_ != 3):
    ...
    nbbpb = []
    o1 = []
    for i in range(v_+1, int(v_+1 + v2/2)):
        o1.append(i)
    o2 = []
    for i in range(int(v_+1+v2/2), v_+v2+1):
        o2.append(i)
...
if is_valid_file(file_path):
    ...
    method6(data, control_treatments)
else:
